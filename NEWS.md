...
Last contribution of Mathilde Boissel as lead developer of this package,
from version 0.1.0 to 0.2.0. 
In this version, development and code optimization were 
performed in collaboration with Claude (Anthropic); 
co-developed, tested and validated by human expertise.

## Contributors

- Add Klervi Le Gall as contributor (`ctb`): bug detection, testing and feedback.

- Add Armand Elegbe as contributor (`ctb`): testing, feedback, and suggestion 
  to add SE and IQR statistics.

## New Features

- Add `SE` (Standard Error of the Mean) and `IQR` (Interquartile Range) 
  as new statistics in `compute_continuous_table()`.

- Add `verbose` parameter across all main functions to control message outputs.

- Add `crossedvarstrat_long_wide` option to control the output format 
  (`"long"` or `"wide"`) when using crossed stratification variables.
  The crossed varstrat situation is now optimized by calling the classic 
  single-varstrat case with a pre-filter on the first varstrat level.

- Add a template `template_report.Rmd` in the `inst/` folder as a usage example.

- Several internal functions have been exported following user requests.

## Improvements

- Improved metric/test alignment: descriptive metric (mean vs median) is now 
  automatically aligned with the statistical test used (parametric vs 
  non-parametric), when `do_test = TRUE`. This prevents inconsistencies such 
  as displaying a t-test result alongside a median, or a Wilcoxon result 
  alongside a mean.

- Default parameter values in `save_excel_paired_results_filtertest()` are now 
  aligned with those of `save_excel_paired_results()`.

## Bug Fixes

- Fix `"NaN +/- NA"` display issue: now correctly replaced by `"/"` when 
  no data is available.

- Fix default parameter values in `save_excel_paired_results_filtertest()` 
  to match `save_excel_paired_results()`.

- Conflict in paired functions when variable names matched internal 
  statistical metric names (`Q1`, `Q3`, `mean`, `median`, `sd`, `min`, `max`).
  A warning is now raised if such naming conflicts are detected in the 
  provided dataset.

- Add a warning when all variables in the dataset are of character type.
...
- In longitudinal context, the internal function `handle_multiple_levels` has
to deal with ANOVA repeated measures from `rstatix::anova_test` function.
  A specific action has been fixed to get back the p-value from this object
  (class "anova_test", "list", and "rstatix_test").
...
In this version only, development and code optimization were 
performed in collaboration with Claude (Anthropic); 
co-developed, tested and validated by human expertise.
...
Intern rule : The mean and median will be rounded to one decimal place higher than
the accuracy of the original value. 
The standard deviation will be rounded to 2 decimal places above the precision
of the original value, 
up to a maximum of 3 decimal places. 
Percentages will be rounded to one decimal place, except for 0% and 100%. 

- Added `do_test` parameter to separate descriptive statistics from statistical 
  testing. When `do_test = FALSE`, only descriptive statistics are returned 
  (P-value and Test columns are omitted).

- Add option to get exact p-values (`show_exact_p`) + fix unit tests.

- Add option `force_parametric_test` : This may be useful when
considering the central limit theorem or small deviations + add unit tests. 
...
- Fix data.table class in global user environment : use copy and setDT (safe control).
...
- Order of the columns corrected to match the breakdown of the 
counts in detail NB (`detail_NB_mesure_sum`), in quanti sheet - according levels order 
...
- Add sheet about date description in `save_excel_results`, 
based on new functions `get_dates` and `compute_date_table` + 
add docs + add corresponding unit tests.
...
- Added Saïd Maallem as contributor (`ctb`): suggestion (dico), testing and feedback.

## New Features

- `dico_mapping` param : provide a dataframe to match vars' names and vars' label.
"Variable" name will be the first column and "Label" will be set as the 2nd column + add test.

- `drop_levels` param : 
drop levels added also on varstrat columns + 
add test in `save_excel_paired_results` and `save_excel_results`

- Remove vars with all NA from the vars selection + with a message. 
Now variables will all Na are removed from the list wanted to be analysed and
just stored in a new sheet "Variables_all_na" if applicable + add tests

- `save_excel_paired_results_filtertest` : new function to use `save_excel_paired_results` 
in case of missing data + add test


## Improvements

- vignette fill with new examples of `save_excel_paired_results` in case of missing data :
understand how work `save_excel_paired_results_filtertest`.

- Change "N_NA" instead of "NA" col name in compute_ function.

## Bug Fixes

- Shapiro.test not executable with less than 3 points 
(in overall population, in bivar mode) + add tests (`compute_continuous_table`)
...
Fix case when a vars has all values equal (no shapiro test possible, capture error)

## Improvements (Style)

- N "total" and N missing clarified and Columns names and types homogenized 
(`Nb_mesures`, `Valeurs_manquantes`, as character)
...
- In `save_excel_paired_results` : About 2 *crossed varstrats* : 
  you can provide "var1*var2", and so
  ...
  var2 as the condition/group that will be tested with *not paired test* 
  (if `crossed_varstrat_test` is set TRUE).
...
- In `compute_paired_continuous_table_and_test` : Fix "cell_content" obj in quade case. 
Fix situation of empty quanti var selected (`dcast` fails after `na.omit` so add early return).
Add messages.

- In `save_excel_paired_results` : When cross varstrats, Fix vars' name when spaces
are replaced by dot in "analyse_desc_quali" (after `compute_factorial_table` step).

- In `save_excel_results` : When cross varstrats, only in quanti sheet, 
when global_summary = TRUE, reorder "Population totale", 
just after missing data, and before groups.

- In `compute_paired_factorial_table_and_test` : 
Set col order in with Vars names and Modalites first.
...
- in general : remove "message" if "No." by nothing ""
...
- new params `signif_digits` : A integer, Default 4. Integer indicating the 
number of decimal places (signif) for pvalues. different from `digits` which
is used to round descriptive statistics.
- "global_summary" option : keep or not stats of "Population_totale"" col in
save_excel functions, like in paired function.

## Bug Fixes

- format : spaces between ";" in "[Q1;Q3]".
- format : consistency ("Population_totale", "Variable", "Nb_mesures",
"P_valeur", "Test", Rename excel sheets)
...
- retirer fonction save_excel initial et n’utiliser que `optim`, 
et la renomer à la place `save_excel_results`.
...
- varstrat_nested =\> “crossed” =\> remove the logical param add instead detection of
"*" in “varstrat” param =\> if “var*var” , go in cross levels selection,
...
- when varstrat continuous, sheet « quali quanti » avec varstrat conti,
retirer « variables », will be in table names. Et simplifier varstrat (ne pas répéter le nom)

- compute correction function : method “auto detect” according shapiro on the 2 vars
=\> if both normal, selection method pearson (remove test lm’s R2 test)
...
- varstrat_nested =\> “crossed” =\> remove the logical param add instead detect of “*” in “varstrat” param =\> if “var*var” , go in cross levels selection,
...
- [BUG]
    ...
    (see <https://cran.r-project.org/web/packages/data.table/vignettes/datatable-importing.html#globals>) :
    automatic creation of "R2Excel-package.R" script.
...
[BUG]
    ...
    - Fix major bugs : fix the test deployed in quali workbook = test_proportion to
    use instead of test_mean (put inadvertently).
    - Fix NAN % by testing is.nan in prop.table =\> show 0% now when a modality is
    not present for a level of varstrat.
