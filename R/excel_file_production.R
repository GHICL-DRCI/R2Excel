...
save_excel_results <- function(
  ...
  if (!do_test) {
    show_SMD <- FALSE
    show_OR <- FALSE
    message("[save_excel_results] do_test = FALSE so show_SMD and show_OR truned FALSE.")
  }
  ...
  if (force_non_parametric_test) {
    message(
      paste0(
        "[save_excel_results] Because force_non_parametric_test is TRUE,",
  ...
  if (force_parametric_test) {
    message(
      paste0(
        "[save_excel_results] Because force_parametric_test is TRUE,",
      ...
      if (!is.null(tab_quali_sheet_tab)) { ## --here v0.1.18
        tab_quali_sheet_tab <- data.table::rbindlist(
          l = lapply(X = unique(tab_quali_sheet_tab$Variable), FUN = function(vari) {
            ...
            # tmp[idx_na, grep(paste0(varstrat[1], ".*_N$"), names(tmp))] <- NA
            
            ## no col_N when cross var strat 
            # Messages d'avis :
            #  Dans `[<-.data.table`(`*tmp*`, idx_na, grep(paste0(varstrat[1], ".*_N$"),  :
...
#' @param show_OR Logical, compute and show Odds Ratios
#' @param show_SMD Logical, compute and show Standardized Mean Differences
#' @param detail_NB_measures Logical, show detailed counts per level
#' @param light_contents Logical, use light formatting (reduce repetitions)
    ...
    tab_quanti_sheet_sep <- lapply(levels_to_sep, function(level_i) {
      ...
      tab_quanti_sheet <- data.table::rbindlist(l = lapply(
        X = seq_len(length(analyse_desc_quanti)), FUN = function(i) {
          ...
          # # wanted or not ? _N # v0.1.27
          # tmp$Nb_mesures <- tmp$Nb_mesures[tmp$Modalites %in% "Population_totale"]
          # and
          # if(detail_NB_measures) {
          #   tmp$Nb_mesures[!tmp$Modalites %in% "Population_totale"]
          #   to pivot and named N_leveli ...
          # }
          ...
          names(tmp2) <- sub(paste0("^", varstrat[2]), paste0(varstrat[2], "="), names(tmp2))
...
quali_sheet <- function(
  ...
  if (crossed_varstrat) {
    ...
    tab_quali_sheet_sep <- lapply(levels_to_sep, function(level_i) {
      message(paste0(
        "[quali_sheet] Go on varstrat ", varstrat[1], 
        " crossed for level ", level_i, "."
      ))
        ...
        tab_quali_sheet <- data.table::rbindlist(
          l = lapply(
            ...
            FUN = function(i) {
              ...
              tmp <- tmp[!Modalites %in% c("Nb_mesures", "Valeurs_manquantes"), 
                         .SD, .SDcols = c(
        ...
        if (do_test) {
          ...
          if (is.numeric(dataframe[[varstrat_i]])) {
            ...
            tab_test <- data.table::rbindlist(lapply(X = vars_quali, FUN = function(quali_i_var) {
              message("[quali_sheet] test_means ", quali_i_var)
              pvaleur_quanti <- test_means(
                dataframe = dataframe,
                vars = varstrat_i,
                varstrat = quali_i_var,
...
#'
#' @param dataframe Data.frame containing the data
#' @param vars_dates Character vector of dates variable names
#' @param varstrat Stratification variable name (can be "", NULL, or a variable name)
#' @param crossed_varstrat Logical
#'
...
date_sheet <- function(
  ...
  if (crossed_varstrat) {
    levels_to_sep <- levels(dataframe[[varstrat[1]]])
    message(paste0("[date_sheet] ", "date (with varstat : ", varstrat[1], ")"))
