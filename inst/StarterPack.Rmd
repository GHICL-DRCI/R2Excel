...
(6) And now, it is possible to describ date variables too : 
...
(9) come with the estimated difference of means
...
Based on `compute_continuous_table` (transposed) table for each qualitative levels and `compute_correlation_table` with it test for quantitative vars (correlation method can be "detect_auto" or classical known approach)
...
(14) You might be interested in putting labels next to your blurry variable names. Use the `dico_labels` param, putting the variable name in the first column and your labels in the second.
...
(D) The option about "cross" 2 varstrats is also available in "paired" function : 
...
(E) Understand `save_excel_paired_results_filtertest` : A specific table for repeated data may be wanted in two ways: 
...
save_excel_paired_results_filtertest(
  ...
  varstrat = "visites_2",
  patient_id = "ID2",
  file = file.path("tmp", "FF-nonmissing_paired_desc.xlsx")
)
```
